...
var header = "/*!\n" +
	...
	" * https://github.com/haxpor/loggiejs/blob/master/LICENSE.md\n" +
	" */\n";
...
function buildDevTask() {
	...
	return gulp.src('dist/loggie-es6.js')
		.pipe(babel({
			presets: ['es2015']
		}))
		.pipe(streamify(concat('loggie-dev-es5.js')))
...
function buildInitialTask() {
	...
	return browserify('./src/loggiejs.js', { standalone: 'loggie' })
		.plugin(collapse)
		.bundle()
		.pipe(source('loggie-es6.js'))
...
function buildNormalTask() {
	...
	return gulp.src('dist/loggie-es6.js')
		...
		.pipe(streamify(replace('{{ version }}', package.version)))
		.pipe(streamify(concat('loggie.js')))
		.pipe(gulp.dest(outDir));
}
...
function buildMinifiedTask() {
	...
	return gulp.src('dist/loggie-es6.js')
		...
		.pipe(streamify(replace('{{ version }}', package.version)))
		.pipe(streamify(concat('loggie.min.js')))
		.pipe(gulp.dest(outDir));
}
